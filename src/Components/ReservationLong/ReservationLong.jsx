...
const ReservationLong = (props) => {
    ...
    const getStylesByStatus = (status) => {
        switch (status) {
            case "ritirato":
                return { iconColor: "#4CAF50", colorDate: "", text: "Ritirato il:" }; // Verde
            case "daRitirare":
                return { iconColor: "#FF9800", colorDate: "#F44336", text: "Da ritirare entro:" }; // Arancione
            case "accettato":
                return { iconColor: "#2196F3", colorDate: "", text: "Accettato il:" }; // Blu
            case "prenotato":
                return { iconColor: "#F3D700", colorDate: "", text: "Prenotato il:" }; // Giallo
            case "rifiutato":
                return { iconColor: "#F44336", colorDate: "", text: "Rifiutato il:" }; // Rosso
            default:
                return { iconColor: "#9E9E9E", colorDate: "", text: "Sconosciuto" }; // Grigio
        }
    };

    // Ottieni il colore e il testo in base ai props
    const { iconColor, colorDate, text } = getStylesByStatus(props.status);
 
    return (
        <div className="reservation-long">

            <svg className="reservation-icon-long"
...
export default ReservationLong
