...
const ReservationLong = (props) => {
    ...
    const { iconColor, colorDate, text, meaning } = getStylesByStatus(props.status);
    ...
    return (
        <div className="reservation-long">
            ...
            {/* Icona con tooltip */}
            <svg
                className={`reservation-icon-long ${props.status === "daRitirare" ? "pulse" : ""}`}
                data-tooltip-id={`tooltip-${props.id}`} // ID unico del tooltip
            ...
            <Tooltip
                id={`tooltip-${props.id}`}
                place="bottom-start" // Posiziona il tooltip in basso
                effect="solid"
                style={{
